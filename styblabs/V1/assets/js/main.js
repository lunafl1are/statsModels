...
if (typeof $ !== "undefined") {
  $(function () {
    ...
    if (searchInput.length) {
      ...
      var searchJson = "/api/v1/data/menu";
      if ($("#layout-menu").hasClass("menu-horizontal")) {
        var searchJson = "search-horizontal.json";
      }
      // Search API AJAX call
      var searchData = $.ajax({
        url: searchJson, //? Use your own search api instead
        dataType: "json",
        async: false,
      }).responseJSON;
      ...
      // Add Active to Selected Menu Item
        ...
        // Add Menu Page Title to Breadcrumb from Menu Item
        var menuTitles = [];
        $("#menu-list li > a").each(function() {
          if ($(this).parent().hasClass("active")) {
            menuTitles.push($(this).text());
          }
        });
        // menuTitles.reverse();fV1
        for (var i = 0;i < menuTitles.length; i++) {
          if(i==menuTitles.length-1){
            $("#menu-breadcrumb").append(menuTitles[i]);
          }else{
          $("#menu-breadcrumb-span").append(menuTitles[i]+" / ");
          }
        }
